# chatserver
2023-03 ~ 2023-07 集群聊天系统 主要负责人 

项目简介：该项目基于 C/S 架构实现的聊天系统，包括注册、登录、一对一/群组聊天、好友管理、群组创建等功能。通过数据、 业务、网络模块的解耦，采用 Json 格式传输数据。服务器集群使用 Nginx 进行负载均衡，借助 Redis 发布-订阅实现服务器间通信。该聊天系统实现了稳定、高效、灵活的通信聊天。 （项目地址：github）
